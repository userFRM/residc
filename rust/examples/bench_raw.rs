...
fn main() {
    ...
    let msgs: Vec<Message> = (0..N_MSGS as u64).map(|i| {
        ts += 1000 + (i * 37 % 50000);
        Message::new()
            .set(0, ts)
            .set(1, (i % 50) as u64)
    ...
    for msg in &msgs {
        ...
        for fi in 0..5 {
            if decoded.get(fi) != msg.get(fi) { errors += 1; break; }
        }
    }

    let raw_size = schema.raw_size();
    println!("residc Rust benchmark (5-field quote, {} messages, best of {} iterations)", N_MSGS, N_ITERS);
    println!("=================================================================");
    println!("  Encode:      {:.0} ns/msg", best_enc);
    println!("  Decode:      {:.0} ns/msg", best_dec);
    println!("  Ratio:       {:.2}:1 ({} -> {:.1} bytes avg)",
